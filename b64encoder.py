...
def uuid_to_base64(u):
    # Ensure the input is a UUID object
    if isinstance(u, str):
        u = uuid.UUID(u)
...
# Function to convert Base64 back to UUID string
def base64_to_uuid(b64_str):
    # Add padding back
    b64_str += '=' * (4 - len(b64_str) % 4)
    # Decode Base64 to bytes
    u_bytes = base64.urlsafe_b64decode(b64_str)
    # Convert bytes back to UUID and return as string
    return str(uuid.UUID(bytes=u_bytes))